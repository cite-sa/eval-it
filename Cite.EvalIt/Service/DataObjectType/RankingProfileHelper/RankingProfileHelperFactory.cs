...
{
    public class RankingProfileHelperFactory<T, IT> where T : struct, IConvertible /*enum constraint*/
                                                                        where IT : class
    {
        public class RankingProfileHelperFactoryConfig
        {
            public Dictionary<T, Type> Accessors { get; } = new Dictionary<T, Type>();

            public RankingProfileHelperFactoryConfig Add(T key, Type type)
            {
        ...
        public RankingProfileHelperFactory(IServiceProvider serviceProvider, Microsoft.Extensions.Options.IOptions<RankingProfileHelperFactoryConfig> config, ErrorThesaurus errors)
        {
