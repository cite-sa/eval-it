...
{
    public class ReviewEvaluationHelperFactory<T, IT> where T : struct, IConvertible /*enum constraint*/
                                                                        where IT : class
    {
        public class ReviewEvaluationHelperFactoryConfig
        {
            public Dictionary<T, Type> Accessors { get; } = new Dictionary<T, Type>();

            public ReviewEvaluationHelperFactoryConfig Add(T key, Type type)
            {
        ...
        public ReviewEvaluationHelperFactory(IServiceProvider serviceProvider, Microsoft.Extensions.Options.IOptions<ReviewEvaluationHelperFactoryConfig> config, ErrorThesaurus errors)
        {
