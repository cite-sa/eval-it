...
namespace Cite.EvalIt.Authorization
{
	public class PermissionPolicyService : IPermissionPolicyService
	{
		...
		public ISet<String> PermissionsOfRoles(IEnumerable<String> roles)
		{
		...
		public ISet<String> PermissionsOfClaims(String claim, IEnumerable<String> values)
		{
			HashSet<String> permissions = new HashSet<String>();
			if (!this._claimPermissionsMap.ContainsKey(claim)) return permissions;
			foreach (String value in values)
			{
				if (!this._claimPermissionsMap[claim].ContainsKey(value)) continue;
				permissions.UnionWith(this._claimPermissionsMap[claim][value]);
			}
			return permissions;
		}
