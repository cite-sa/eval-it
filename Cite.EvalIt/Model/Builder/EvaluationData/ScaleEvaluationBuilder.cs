...
{
    public class ScaleEvaluationBuilder : Builder<ReviewEvaluation, Data.ReviewEvaluation>
    {
        public ScaleEvaluationBuilder(
        ...
        public override Task<List<ReviewEvaluation>> Build(IFieldSet fields, IEnumerable<Data.ReviewEvaluation> datas)
        {
            ...
            foreach (Data.ScaleEvaluation d in datas)
            {
                ScaleEvaluation m = new ScaleEvaluation();
                if (fields.HasField(this.AsIndexer(nameof(ScaleEvaluation.Values)))) m.Values = d.Values;
