...
import { UserWithRelationshipModel } from '@app/ui/my-network/my-network-editor/my-network-editor.model';
...
@Component({
  selector: 'app-my-network-editor',
  templateUrl: './my-network-editor.component.html',
  styleUrls: ['./my-network-editor.component.css']
})
export class MyNetworkEditorComponent extends BaseEditor<UserWithRelationshipModel, UserWithRelationship> implements OnInit {
	...
	ngOnInit(): void {
		this.route.url.pipe(switchMap(data => {
			...
			this.userId = Guid.parse(data[0].path)
			...
			let network = this.userService.getMyNetwork(userFields);
			let users = this.userService.query(userLookup);
			return forkJoin([network,users]);
		}))
		.subscribe(res => {
			this.usersInNetwork = res[0].items.map(t => t.user);
			this.users = res[1].items;
			
			if( !this.userId.equals(Guid.parse("0")) )
			{
				var pair = res[0].items.find( x => x.id == this.userId)
	...
	public delete() {
		...
		if (value.id) {
			...
			dialogRef.afterClosed().pipe(takeUntil(this._destroyed)).subscribe(result => {
				if (result) {
					this.userService.myNetworkRemove(value).pipe(takeUntil(this._destroyed))
						.subscribe(
	...
	refreshOnNavigateToData(id?: Guid): void {
		if (this.isNew) {
			...
			const addr = '/my-network/' + this.formGroup.value.id ?? '';
	...
	persistEntity(onSuccess?: (response) => void) : void {
		...
		this.userService.myNetworkAdd(this.formGroup.getRawValue(), userFields)
	...
	buildForm() {
		this.formGroup = this.editorModel.buildForm(null, this.isNew, false);
	}


	isUserDisabled(userId : Guid): boolean {
		// TODO: Maybe Optimize
		if ( this.usersInNetwork.map(x => x.id).includes(userId) ) return true;
		return false;
	}

	getPair(itemId: Guid, userId: Guid, successFunction: (item: UserWithRelationship) => void): void {
		this.userService.getSingle(itemId, MyNetworkEditorResolver.lookupFields())
