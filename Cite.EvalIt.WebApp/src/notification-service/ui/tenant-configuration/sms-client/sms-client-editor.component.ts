...
import { SmsClientConfigurationDataContainer, TenantConfiguration } from '@notification-service/core/model/tenant-configuration.model';
...
import { TenantConfigurationSmsClientEditorModel } from '@notification-service/ui/tenant-configuration/sms-client/sms-client-editor.model';
import { takeUntil } from 'rxjs/operators';
import { nameof } from 'ts-simple-nameof';

@Component({
	selector: 'app-tenant-configuration-sms-client-editor',
	templateUrl: './sms-client-editor.component.html',
	styleUrls: ['./sms-client-editor.component.scss']
})
export class SmsClientEditorComponent extends TenantConfigurationBaseComponent implements OnInit {

	formGroup: FormGroup;
	editorModel: TenantConfigurationSmsClientEditorModel;
	...
	getConfiguration() {
		...
		lookup.type = [TenantConfigurationType.SmsClientConfiguration];
		...
		lookup.project = {
			fields: [
				...
				nameof<TenantConfiguration>(x => x.smsClientData) + '.' + nameof<SmsClientConfigurationDataContainer>(x => x.applicationId),
				nameof<TenantConfiguration>(x => x.smsClientData) + '.' + nameof<SmsClientConfigurationDataContainer>(x => x.applicationSecret),
				nameof<TenantConfiguration>(x => x.smsClientData) + '.' + nameof<SmsClientConfigurationDataContainer>(x => x.sendAsName),
			]
		};
		this.tenantConfigurationService.query(lookup)
			...
			.subscribe(
				data => {
					try {
						this.editorModel = data.items.length > 0 ? new TenantConfigurationSmsClientEditorModel().fromModel(data.items[0]) : new TenantConfigurationSmsClientEditorModel();
	...
	private persist(totp?: string) {
		this.tenantConfigurationService.persistSmsClientConfiguration(this.formGroup.value, totp)
			.pipe(takeUntil(this._destroyed))
			.subscribe(
